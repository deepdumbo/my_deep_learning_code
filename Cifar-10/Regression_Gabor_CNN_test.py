...
def conv2d_add_Gabor(input, name, kernel_size, input_channel, output_channel, Gabor_filter):
    Gabor_shape = Gabor_filter.get_shape()
    Gabor_channel = Gabor_shape[3]
    W = tf.get_variable(name=name + '_Weight', shape=[kernel_size, kernel_size, input_channel, output_channel - Gabor_channel])
    W_ = tf.concat([W, Gabor_filter], axis = -1)
    b = tf.get_variable(name=name + '_bias', shape=[output_channel])
    return tf.add(tf.nn.conv2d(input, W_, strides=[1, 1, 1, 1], padding='SAME'), b)
...
R_act2 = tf.nn.relu(R_conv2)
R_pool2 = max_pooling_2d(input=R_act2, width=2, height=2)

R_theta = fc(input = tf.reshape(R_pool2, [batch_size, 8 * 8 * 128]), name = 'R_theta', input_channel = 8 * 8 * 128, output_channel = 4)
R_lambda = fc(input = tf.reshape(R_pool2, [batch_size, 8 * 8 * 128]), name = 'R_lambda', input_channel = 8 * 8 * 128, output_channel = 4)
