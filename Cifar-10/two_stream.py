...
class batch_norm(object):
    ...
    def __call__(self, x, train=True):
        ...
        with tf.variable_scope('batch_norm'+str(cnt)) as scope:
            global cnt
            cnt = cnt + 1
            if train is not None:
                self.beta = tf.get_variable("beta"+str(cnt), [shape[-1]],
                                            initializer=tf.constant_initializer(0.))

                self.gamma = tf.get_variable("gamma"+str(cnt), [shape[-1]],
...
#Gabor
pi = tf.asin(1.0) * 2.0
Theta = tf.Variable(tf.linspace(0.0, pi, 4))
#print Theta.eval()
...
W_conv1 = weight_variable([3, 3, 3, 36])
...
W_conv1_gabor = weight_variable([3, 3, 16, 36])
b_conv1_gabor = bias_variable([36])

conv1_gabor = convolution_2d(Gabor_conv, W_conv1_gabor, b_conv1_gabor)
act1_gabor = tf.nn.relu(batch_norm()(conv1_gabor, train = BN_train))
pool1_gabor = max_pooling(act1_gabor, 2, 2)
drop1_gabor = tf.nn.dropout(pool1_gabor, keep_prob)
...
W_conv2_gabor = weight_variable([3, 3, 36, 128])
b_conv2_gabor = bias_variable([128])

conv2_gabor = convolution_2d(drop1_gabor, W_conv2_gabor, b_conv2_gabor)
act2_gabor = tf.nn.relu(batch_norm()(conv2_gabor, train = BN_train))
pool2_gabor = max_pooling(act2_gabor, 2, 2)
drop2_gabor = tf.nn.dropout(pool2_gabor, keep_prob)
...
W_conv3_gabor = weight_variable([3, 3, 128, 256])
b_conv3_gabor = bias_variable([256])

conv3_gabor = convolution_2d(drop2_gabor, W_conv3_gabor, b_conv3_gabor)
act3_gabor = tf.nn.relu(batch_norm()(conv3_gabor, train = BN_train))
pool3_gabor = max_pooling(act3_gabor, 2, 2)
drop3_gabor = tf.nn.dropout(pool3_gabor, keep_prob)
...
y_predict_norm = tf.nn.softmax(tf.matmul(drop, W_fc2) + b_fc2)

W_fc1_gabor = weight_variable([4 * 4 * 256, 256])
b_fc1_gabor = bias_variable([256])

flat_gabor = tf.reshape(drop3_gabor, [-1, 4 * 4 * 256])
fc1_gabor = tf.nn.relu(batch_norm()((tf.matmul(flat_gabor, W_fc1_gabor) + b_fc1_gabor), train=BN_train))
drop_gabor = tf.nn.dropout(fc1_gabor, keep_prob)

W_fc2_gabor = weight_variable([256, 10])
b_fc2_gabor = bias_variable([10])

y_predict_gabor = tf.nn.softmax(tf.matmul(drop_gabor, W_fc2_gabor) + b_fc2_gabor)


#fusion = tf.Variable(0.5)


y_predict = (y_predict_gabor + y_predict_norm) / 2
